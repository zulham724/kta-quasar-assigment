    ...
    <q-list bordered class="rounded-borders" v-if="assigment" style="border:1px solid #1976D2;">
        <q-expansion-item expand-separator :label="assigment.subject">
            ...
            <q-card>
                <q-card-section>
                    <q-list>
                        <q-item v-for="(question_list, ql) in assigment.question_lists" :key="`${ql}-${assigment.id}-${question_list.id}`">
                            <q-item-section>
                                <q-item-label>
                                    ...
                                    <div class="row justify-center">
                                        <q-btn v-if="ruleAddQuestionList(assigment)" flat @click="addQuestionList(question_list)">Pilih Soal {{ ql + 1 }}</q-btn>
                                    </div>
                                </q-item-label>
                            </q-item-section>
                        </q-item>
...
export default {
    ...
    methods: {
        ...
        ruleAddQuestionList(assigment) {
            ...
            if (
                assigment.assigment_category_id ==
