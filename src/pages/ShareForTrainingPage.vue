      ...
      <q-toolbar class="bg-blue">
        <q-btn
          ...
          @click="
            $router.push({
              name: 'account',
              params: { tabName: 'publish', assigmentIdtoScroll: assigmentId }
            })
          "
        />
        <q-toolbar-title>
...
export default {
  ...
  //   },
  created() {
    console.log("assigment shared for training", this.assigment);
    if (this.assigment == null) {
      this.$router.back();
      return;
    }
    this.assigmentUpdate = {
      ...this.assigment,
      isTimer: this.assigment.timer
        ? true
        : false /*, is_public:this.assigment.is_public?true:false*/
    };
    ...
    onSubmit() {
      this.$refs.form.validate().then(success => {
        if (success) {
          ...
          axios
            .post(`${this.Setting.url}/api/v1/assigments/setpublic`, {
              id: this.assigment.id,
              ...this.assigmentUpdate
            })
            .then(res => {
              this.$store.dispatch("Auth/getAuth");
              if (!this.assigment.is_public)
                this.$q.notify("Berhasil mempublikasikan paket soal ke siswa");
              else
                this.$q.notify(
                  "Berhasil mengedit paket soal yang dipublikasikan ke siswa"
                );

              const keys = ["note", "timer","is_public"];
              this.$store.commit("Auth/setAssigmentByKey", {
                assigment: res.data,
                keys
              });
              ...
              this.$router.push({
                ...
                params: {
                  tabName: "publish",
                  assigmentIdtoScroll: this.assigmentId
                }
              });
            });
        }
