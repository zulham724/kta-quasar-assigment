...
<div>
    <q-header elevated>
        <q-toolbar class="bg-blue">
            <q-toolbar-title>
                <div @click="$router.push({ name: 'payable_build', params: { _step: 2 } })" class="text-right text-body1">Lanjut</div>
            </q-toolbar-title>
            <!--<q-icon name="keyboard_backspace" style="font-size: 1.5em;" clickable @click="$router.push({ name: 'build', params: { _step: 2 } })" />-->
            <q-btn flat round dense icon="arrow_right_alt" @click="$router.push({ name: 'payable_build', params: { _step: 2 } })" />
        </q-toolbar>
    </q-header>
                ...
                <q-infinite-scroll @load="onLoad" :offset="250">
                    <div class="q-pa-none q-gutter-y-xs">
                        <div class="row q-gutter-sm">
                            <div class="col">
                                <q-input v-model="search" dense placeholder="Cari topik soal" filled type="search" hint="contoh: rukun islam" @input="onSearch">
                            ...
                            <div class="col">
                                <q-select dense label="Tampilkan soal" :options="options" option-value="id" option-label="description" v-model="grade" />
                            </div>
                        </div>
                        ...
                        <div v-if="!SuggestedPayableQuestionList.items.data">
                        ...
                        <q-intersection v-for="assigment in getFilteredQuestionList" :style="`min-height:70px;min-width:100%`" :key="assigment.id">
...
export default {
    ...
    computed: {
        ...mapState([
            ...
            "Setting",
            "Assigment",
            "SuggestedPayableQuestionList"
        ]),
        getFilteredQuestionList() {
            if (this.grade.id == 0) return this.SuggestedPayableQuestionList.items.data;
            else return this.SuggestedPayableQuestionList.items.data.filter(item => {
    ...
    created() {
        ...
        this.getSuggestedPayableQuestionLists();
    ...
    methods: {
        debounce,
        onSearch() {
            this.getSuggestedPayableQuestionLists(this.search);
        },
        step2() {
            this.init();
            this.getSuggestedPayableQuestionLists();
            ...
            this.assigment.question_lists.push({
                ...
                pivot: {
                    creator_id: question_list.assigments.length ?
                        question_list.assigments[0].pivot.creator_id : null,
                    user_id: question_list.assigments.length ?
                        question_list.assigments[0].pivot.user_id : null,
                    assigment_type: question_list.assigments.length ?
                        question_list.assigments[0].pivot.assigment_type : null,
                    assigment_type_id: question_list.assigments.length ?
                        question_list.assigments[0].pivot.assigment_type_id : null
                },
        ...
        getSuggestedPayableQuestionLists(search) {
            if (
            ...
            this.$store.dispatch("SuggestedPayableQuestionList/index", {
        ...
        onLoad(index, done) {
            this.SuggestedPayableQuestionList.items.next_page_url ?
                this.$store.dispatch("SuggestedPayableQuestionList/next").then(res => done()) :
                done();
        }
    }
};
