<template>
  <div>
        ...
        >
          ...
          <q-stepper-navigation>
            <div class="row justify-between">
              <q-btn flat dense @click="step = 1" color="primary" label="Back" class="q-ml-sm" />
              <q-fab
                flat
                color="secondary"
                label="Butir soal"
    ...
    <q-dialog v-model="search.display">
      <q-card>
        <q-card-section>
          <div class="text-h6">Cari Soal</div>
        </q-card-section>
...
export default {
  ...
  computed: {
    ...mapState(["Grade", "Auth", "AssigmentCategory", "Setting", "Assigment"])
  },
  watch: {
    assigment: {
      handler: function() {
  ...
  created() {
    if (this.Assigment.build) this.assigment = {...this.Assigment.build};
