...
<div>
    ...
    <q-page>
        <q-stepper header-nav v-model="step" vertical color="primary" animated>
            ...
            <q-step v-for="(question, qi) in session.questions" :key="question.id" :name="qi+1" :title="`Soal nomor ${qi + 1}`" icon="settings" :done="step > qi+1" :caption="getPoint(question)" :error="question.score==null">
                ...
                <div v-if="
                  question.assigment_question_list.assigment_type.description ==
                    'selectoptions'
                ">
                    <q-input :bg-color="answer_list.value ? 'green-4' : question.answer.answer_list_id == answer_list.id ? 'red-4' : null" rounded disable outlined dense autogrow v-for="answer_list in question.answer_lists" :key="`answer-${answer_list.id}`" v-model="answer_list.name" :rules="[val => !!val || 'Harus diisi']">
                        <template v-slot:after>
                            <div>
                                <!--<q-btn round icon="check" :flat="false" color="green" v-if="answer_list.value==100" />-->
                                <q-btn round icon="close" :flat="false" color="red" v-if="answer_list.value==null && answer_list.id==question.answer.answer_list_id" />
                            </div>
                        </template>
                    </q-input>
                </div>
                ...
                ">
                    <q-input v-for="(answer_list, key) in question.answer_lists" :key="key" class="q-pa-sm" outlined dense disable :label="`Kunci Jawaban ${key+1}`" v-model="answer_list.name" />
                    <q-input class="q-pa-sm" outlined dense disable label="Jawaban Siswa" v-model="question.answer.name" />
                </div>

                <div v-if="
                  question.assigment_question_list.assigment_type.description == 'textarea'
                ">
                    <q-input v-for="(answer_list, key) in question.answer_lists" :key="key" class="q-pa-sm" outlined dense disable type="textarea" :label="`Kunci Jawaban ${key+1}`" v-model="answer_list.name" />
                    <q-input class="q-pa-sm" outlined dense disable type="textarea" label="Jawaban Siswa" v-model="question.answer.name" />
                </div>
                ...
                <div class="row q-pt-sm">
                    ...
                    <div class="col-4 q-pl-sm column">
                        <q-btn v-if="qi != 0" flat @click="step = step - 1" color="primary" label="Kembali" class="q-ml-sm" />
                        <q-btn v-if="qi + 1 != session.questions.length" outline rounded @click="[step++]" color="primary" label="Lanjut" />
                        <q-btn v-if="qi + 1 == session.questions.length" outline rounded @click="submitScore" color="primary" label="Selesai" :loading="loading" :disable="loading" />
                    </div>
