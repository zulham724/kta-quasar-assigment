    ...
    <q-layout view="lHh Lpr lFf">
        ...
        <q-page-container v-if="assigment != null">
            <q-form @submit="onSubmit" ref="form" class="full-height">
                ...
                ">
                            <q-input class="q-pa-sm" rounded outlined dense v-model="question_list.answer" :rules="[(val) => !!val || 'Harus diisi']" />
                        </div>

                        <div v-if="
                  question_list.pivot.assigment_type.description == 'textarea'
                ">
                            <q-input class="q-pa-sm" type="textarea" rounded outlined dense v-model="question_list.answer" :rules="[(val) => !!val || 'Harus diisi']" />
                        </div>

                        <q-stepper-navigation class="row justify-end">
                            <q-btn v-if="ql != 0" flat @click="step = step - 1" color="primary" label="Kembali" class="q-ml-sm" />
                            <q-btn v-if="ql + 1 != assigment.question_lists.length" outline rounded @click="step++" color="primary" label="Lanjut" />
                            <q-btn v-if="ql + 1 == assigment.question_lists.length" outline rounded @click="onSubmit" color="primary" label="Selesai" :loading="loading" :disable="loading" />
        ...
        onSubmit() {
            this.$refs.form.validate().then((success) => {
                if (
                    ...
                    console.log(this.assigment.question_lists.filter((item) =>
                        !item.answer ? item : null
                    ));
