        ...
        <div id="corner1" style="margin-top: 10px" class="bg-blue-6">
          <div class="q-pt-sm q-pl-md">
            <div class="text-h5 text-white">
             ...
             <div class="text-h6 text-white">
              {{ assigment.topic ? assigment.topic : "" }}
            </div>
            <div class="text-white">
              {{ assigment.grade ? assigment.grade.description : "" }} Semester {{ assigment.semester ? assigment.semester : "" }} Tahun ajaran {{ assigment.education_year ? assigment.education_year : "" }}
            </div>
            ...
            <div class="text-white">
              {{ assigment.code ? assigment.code : ""
                ...
                icon="content_copy"
              />
              <q-btn no-caps dense outline rounded @click="downloadExcel" color="white"
                ><span class="text-caption q-px-xs">Download Excel</span></q-btn
              >
            </div>
          </div>
          ...
          <div id="corner2" class="bg-blue-4">
            ...
            <div id="corner3" class="bg-blue-1">
              <div class="text-h5 q-pa-sm">Nilai Siswa</div>

              <q-infinite-scroll @load="onLoad" :offset="250" v-if="sessions.length > 0">
                <q-intersection
...
export default {
  ...
  computed: {
    ...mapState(["Setting", "Auth","StudentAssigment"]),
    maxScore() {
      return this.assigment.max_score != null ? this.assigment.max_score : 0;
    },
    avgScore() {
      return this.assigment.avg_score != null ? Math.floor(this.assigment.avg_score) : 0;
    },
  },
  created() {
    this.init();
  ...
  methods: {
    init() {
      this.$store
        .dispatch("StudentAssigment/getAssigment", {
          assigment_id: this.assigmentId,
          isRefresh: false,
        })
        .then((res) => {
          this.assigment = res;
      ...
      this.$store
        .dispatch("StudentAssigment/index", {
          assigment_id: this.assigmentId,
          isRefresh: false,
        })
        .then((res) => {
    ...
    onRefresh(done) {
      Promise.all([
        ...
        this.$store.dispatch("StudentAssigment/getAssigment", {
          assigment_id: this.assigmentId,
          isRefresh: true,
        }),
    ...
    onLoad(index, done) {
      let current_session = this.StudentAssigment.items.find(
        (e) => e.assigment_id == this.assigmentId
      );
      if (current_session) {
        current_session.sessions.next_page_url
          ? this.$store.dispatch("StudentAssigment/next", current_session).then((res) => {
              this.sessions = this.StudentAssigment.items.find(
                (e) => e.assigment_id == this.assigmentId
              ).sessions.data;
              done();
            })
    ...
    downloadExcel() {
      //window.
      const url=`${this.Setting.url}/api/v1/assigments/${this.assigmentId}/${this.Auth.auth.id}/downloadexcel`;
